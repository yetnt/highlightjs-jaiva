...
const yourLanguage = require("../../src/languages/jaiva");
const path = require("path");
const utility = require("../utility");

const languageName = "jaiva";
...
describe("hljs.highlightAuto()", () => {
  ...
  it(`should be detected as ${languageName}`, async () => {
    ...
    await Promise.all(
      filenames.map(async function (example) {
        const filename = path.join(languagePath, example);

        const content = await fs.readFile(filename, "utf-8");
        const detectedLanguage = hljs.highlightAuto(content).language;

        detectedLanguage.should.equal(
          languageName,
          `${path.basename(
            filename
          )} should be detected as ${languageName}, but was ${detectedLanguage}`
        );
      })
    );
  });
